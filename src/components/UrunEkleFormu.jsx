...
export default function UrunEkleFormu({ 
  ...
  const handleTopluMetinIsle = (hamMetin) => {
    ...
    for (let satirMetni of satirlar) {
      ...
      for (let s of sutunlar) {
        ...
        if (sUst.includes("MM") || sUst.includes("CAM") || sUst.includes("TEMPER") || sUst.includes("RODAJ") || sUst.includes("ISICAM") || sUst.includes("LAMİNE") || sUst.includes("SATİNE")) {
          if (!algilananCamAdi) algilananCamAdi = s;
          continue;
        }
  ...
  const anaSatirOlusturHelper = (hedefEn, hedefBoy, hedefManuelM2, hedefMiktar, hedefBirim, hedefPozNo, hedefSecili, secilenSonUrun, hedefOzelAciklama = null) => {
    ...
    satir.hamVeri = {
      ...
      secilenId, 
      pozNo: hedefPozNo, 
      ozelAciklama: kullanilacakOzelAciklama, 
      en: enDegeri, 
      boy: boyDegeri, 
      manuelM2: manuelM2Degeri, 
      miktar: miktarDegeri, 
      secilenBirim: hedefBirim, 
      fiyatAna, 
      fiyatAdet, 
      paraBirimi, 
  ...
  return (
    <section className="panel" style={{ backgroundColor: "white", padding: "18px", borderRadius: "8px", border: "1px solid #cbd5e1", marginBottom: "20px", boxShadow: "0 2px 4px rgba(0,0,0,0.04)" }}>
      ...
      {/* --- FİYATLANDIRMA SATIRI --- */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 100px", gap: "10px", marginBottom: "12px", alignItems: "flex-end" }}>
        <div>
